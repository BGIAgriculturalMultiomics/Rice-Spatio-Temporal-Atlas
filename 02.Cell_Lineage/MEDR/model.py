...
class MEDR(nn.Module):
    def __init__(
        ...
        # self.decoder = GATConv(self.latent_dim, self.input_dim, heads=1, concat=False, dropout=0, add_self_loops=False, bias=False)
        ...
        self.atten = AttentionLayer(self.gcn_hidden2, self.gcn_hidden2)
    ...
    def forward(self, x, adj, pre_adj):
        ...
        pre_mu, pre_logvar, pre_feat_x = self.encode(x, pre_adj)
        pre_gnn_z = self.reparameterize(pre_mu, pre_logvar)

        emb_z, alpha = self.atten(gnn_z, pre_gnn_z)

        # emb_z = 0.7 * gnn_z + 0.3 * pre_gnn_z

        z = torch.cat((feat_x, emb_z), 1)


        de_feat = self.decoder(z, adj)

        pre_de_feat = self.decoder(z, pre_adj)
        ...
        pre_recon = pre_de_feat.clone()  #
        x_init = x[mask_nodes]  #
        pre_x_rec = pre_recon[mask_nodes]  #
        pre_loss = self.criterion(pre_x_rec, x_init)  #

        z1 = torch.cat((feat_x, gnn_z), 1)




        return z, de_feat, pre_de_feat, loss, pre_loss, mu, logvar, pre_mu, pre_logvar, z1
